...
const DEFAULT_MANIFEST_CACHE_TTL_MS = 5 * 60 * 1000; // 5 minutes
...
export async function getManifest(
  env: Env,
  cache: Cache | null,
  manifestCacheTTLms: number = DEFAULT_MANIFEST_CACHE_TTL_MS
): Promise<PageFeatureMapping> {
  const now = Date.now();

  // Check in-memory cache first
  if (manifestCache && now - manifestCacheTimestamp < manifestCacheTTLms) {
  ...
  if (cache) {
    const response = new Response(JSON.stringify(manifest), {
      headers: {
        ...
        'Cache-Control': `public, max-age=${manifestCacheTTLms / 1000}`,
      },
    });
...
export async function getFeatureKeyForPath(
  ...
  const manifest = await getManifest(env, cache, manifestCacheTTLms);
